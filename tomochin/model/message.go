...
func MessagesAll(db *sql.DB) ([]*Message, error) {
	...
	for rows.Next() {
		m := &Message{}
		// Tutorial 1-2. ユーザー名を表示しよう
		if err := rows.Scan(&m.ID, &m.Body, &m.User); err != nil {
			return nil, err
		}
		ms = append(ms, m)
	}
...
func (m *Message) Insert(db *sql.DB) (*Message, error) {
	...
	res, err := db.Exec(`insert into message (body, username) values (?, ?)`, m.Body, m.User)
	if err != nil {
		return nil, err
	}
...
func UpdateMessageByID(db *sql.DB, msg *Message, newBody string) (*Message, error) {
	fmt.Printf("update1")
	_, err := db.Exec(`update message set body=? where id=?`, newBody, msg.ID)
	if err != nil {
		return nil, err
	}
	fmt.Printf("update2")
...
func DeleteMessageByID(db *sql.DB, id int64) error {
	fmt.Printf("delete1")
